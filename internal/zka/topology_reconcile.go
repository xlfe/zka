...
const presentationCaptureSettle = 250 * time.Millisecond

// beginPresentationCaptureSuppression drops watcher feedback generated by
// ZKA's own user-var and title writes. The short settle window covers delivery
// through Kitty's watcher socket without holding off ordinary capture for the
// duration of a topology reconciliation.
func (d *Daemon) beginPresentationCaptureSuppression(endpoint string) func() {
	d.topologyMu.Lock()
	d.captureHold[endpoint]++
	d.topologyMu.Unlock()
	return func() {
		...
		until := time.Now().Add(presentationCaptureSettle)
		if d.captureHoldUntil[endpoint].Before(until) {
			d.captureHoldUntil[endpoint] = until
		}
		d.topologyMu.Unlock()
